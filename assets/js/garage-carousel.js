...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateInfoPanel = (targetIndex) => {
        ...
        if (carData.images && Array.isArray(carData.images) && carData.images.length > 0) {
            carData.images.forEach(imgData => {
                ...
                // Store data needed for the lightbox on the clickable element
                itemDiv.dataset.src = imgData.url || '';
                itemDiv.dataset.title = imgData.title || '';
                itemDiv.dataset.description = imgData.description || '';

                const img = document.createElement('img');
                // Use relative paths directly as generated by Jekyll's relative_url filter
                img.src = imgData.url ? `${imgData.url}` : '/assets/images/placeholder.png';
    ...
    const openLightbox = (imageSrc, imageTitle, imageDescription) => {
        ...
        lightboxImage.setAttribute('alt', imageTitle || 'Enlarged image'); // Use title for alt

        lightboxTitle.textContent = imageTitle || '';
        lightboxDescription.textContent = imageDescription || '';

        // Optionally hide caption area if empty
